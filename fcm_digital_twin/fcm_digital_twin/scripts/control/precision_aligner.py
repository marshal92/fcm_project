...
class PrecisionAligner(Node):
    ...
    def normalize_angle(self, angle):
        """Normalization of the angle to the limits [-pi, pi]"""
    ...
    def goal_cb(self, msg):
        ...
        self.get_logger().info("Received goal for precision alignment! Starting parking.")
    ...
    def control_loop(self):
        ...
        if self.state == 'TURN_TO_GOAL':
            ...
            if distance > self.xy_tol:
                ...
                cmd.linear.x = max(min(cmd.linear.x, 0.2), 0.05) 
            ...
            if abs(error_yaw) > self.yaw_tol:
                ...
                self.get_logger().info("Precision alignment completed successfully!")
                self.active = False
                self.state = 'DONE'
